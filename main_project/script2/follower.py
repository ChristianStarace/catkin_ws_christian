...
class Follower:
    def __init__(self):
        ...
        self.leader_path = deque(maxlen=100)  # Coda FIFO per seguire la traiettoria del leader
        self.cmd_pub = rospy.Publisher("/mir_follower/mobile_base_controller/cmd_vel", Twist, queue_size=100)
    ...
    def follow_leader(self):
        while not rospy.is_shutdown():
            ...
            if leader_movement < 0.02 and distance < 4.0:
            #if leader_movement < 0.02:    
                cmd.linear.x = 0.0
                cmd.angular.z = 0.0  
            else:
                ...
                elif distance < 4.0 and distance >= 1.0:
                    cmd.linear.x = 0.1 * distance                
                else:
                    cmd.linear.x = 0.05 * distance
